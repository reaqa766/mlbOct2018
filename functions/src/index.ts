    ...
    for (const tipo of tipoLigas) {
      ...
      for (const liga of ligas) {
        ...
        for (const equipo of liga.equipos) {
          ...
          const { data: jugadoresAPI } = await axios.get(`https://statsapi.mlb.com/api/v1/people?personIds=${personIds}&season=2021&hydrate=stats(type=gameLog,season,season=2021,gameType=R)`)
